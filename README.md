...
* `app/sync-service.js` is a long-running HTTP service that wraps `sync.js`. It exposes `/health`, `/sync`, `/positions`, and `/transactions` endpoints, runs the cron schedules, and spawns `sync.js` as a child process. A `running` flag prevents overlapping syncs — a trigger received while a sync is in flight is skipped, not queued.
* `app/render-tables.js` backs the `/positions-table` and `/transactions-table` endpoints, which return the same data as browser-readable, click-to-sort HTML tables (self-contained page, inline CSS/JS, no external assets) with summary cards. Handy for eyeballing the portfolio without piping JSON through a formatter.
* `app/portfolio.js` backs the read-only `/positions` and `/transactions` endpoints. These pull live from SnapTrade (no database, independent of the sync schedule) and return JSON. `GET /positions` lists current holdings (account, ticker, shares, average purchase price per share, total cost basis, current price, market value, open P/L). `GET /transactions` adds buy dates (and per-lot purchase prices) and a `realizedSells` list — closed/partial sells with date, shares, sell price, proceeds, matched cost basis, and realized gain, computed by FIFO-matching sell activities against buy history. Realized gain is best-effort and only as complete as the activity history the brokerage exposes (incomplete matches are flagged `costBasisComplete: false`); money-market/cash-equivalent sweeps are excluded.
