...
* `app/portfolio.js` backs the read-only `/positions` and `/transactions` endpoints. These pull live from SnapTrade (no database, independent of the sync schedule) and return JSON. `GET /positions` lists current holdings (account, ticker, shares, average purchase price, cost basis, current price, market value). `GET /transactions` adds reconstructed open-lot buy dates and a `realizedSells` list — closed/partial sells with date, shares, sell price, proceeds, matched cost basis, and realized gain, computed by FIFO-matching sell activities against buy history. Realized gain is best-effort and only as complete as the activity history the brokerage exposes (incomplete matches are flagged `costBasisComplete: false`).
